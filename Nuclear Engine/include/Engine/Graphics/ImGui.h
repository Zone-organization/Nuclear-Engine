...
#define IMGUI_CHECKVERSION()        NuclearEngine::Graphics::ImGui::DebugCheckVersionAndDataLayout(IMGUI_VERSION, sizeof(ImGuiIO), sizeof(ImGuiStyle), sizeof(ImVec2), sizeof(ImVec4), sizeof(ImDrawVert), sizeof(ImDrawIdx))
...
namespace NuclearEngine
{
	namespace Graphics 
	{
		namespace ImGui
		{
			...
			IMGUI_API void          SetTooltip(const char* fmt, ...) IM_FMTARGS(1);                     // set a text-only tooltip, typically use with NuclearEngine::Graphics::ImGui::IsItemHovered(). override any previous call to SetTooltip().
			...
			IMGUI_API ImVec2        GetMousePos();                                                      // shortcut to NuclearEngine::Graphics::ImGui::GetIO().MousePos provided by user, to be consistent with other calls
			...
			IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in NuclearEngine::Graphics::ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
// Flags for NuclearEngine::Graphics::ImGui::Begin()
...
// Flags for NuclearEngine::Graphics::ImGui::InputText()
...
// Flags for NuclearEngine::Graphics::ImGui::TreeNodeEx(), NuclearEngine::Graphics::ImGui::CollapsingHeader*()
...
// Flags for NuclearEngine::Graphics::ImGui::Selectable()
...
// Flags for NuclearEngine::Graphics::ImGui::BeginCombo()
...
// Flags for NuclearEngine::Graphics::ImGui::BeginTabBar()
...
// Flags for NuclearEngine::Graphics::ImGui::BeginTabItem()
...
// Flags for NuclearEngine::Graphics::ImGui::IsWindowFocused()
enum ImGuiFocusedFlags_
{
    ImGuiFocusedFlags_None                          = 0,
    ImGuiFocusedFlags_ChildWindows                  = 1 << 0,   // IsWindowFocused(): Return true if any children of the window is focused
    ImGuiFocusedFlags_RootWindow                    = 1 << 1,   // IsWindowFocused(): Test from root window (top most parent of the current hierarchy)
    ImGuiFocusedFlags_AnyWindow                     = 1 << 2,   // IsWindowFocused(): Return true if any window is focused. Important: If you are trying to tell how to dispatch your low-level inputs, do NOT use this. Use NuclearEngine::Graphics::ImGui::GetIO().WantCaptureMouse instead.
    ImGuiFocusedFlags_RootAndChildWindows           = ImGuiFocusedFlags_RootWindow | ImGuiFocusedFlags_ChildWindows
};

// Flags for NuclearEngine::Graphics::ImGui::IsItemHovered(), NuclearEngine::Graphics::ImGui::IsWindowHovered()
...
// Flags for NuclearEngine::Graphics::ImGui::BeginDragDropSource(), NuclearEngine::Graphics::ImGui::AcceptDragDropPayload()
...
// Enumateration for NuclearEngine::Graphics::ImGui::SetWindow***(), SetNextWindow***(), SetNextItem***() functions
...
#define IM_ALLOC(_SIZE)                     NuclearEngine::Graphics::ImGui::MemAlloc(_SIZE)
#define IM_FREE(_PTR)                       NuclearEngine::Graphics::ImGui::MemFree(_PTR)
#define IM_PLACEMENT_NEW(_PTR)              new(ImNewDummy(), _PTR)
#define IM_NEW(_TYPE)                       new(ImNewDummy(), NuclearEngine::Graphics::ImGui::MemAlloc(sizeof(_TYPE))) _TYPE
template<typename T> void IM_DELETE(T* p)   { if (p) { p->~T(); NuclearEngine::Graphics::ImGui::MemFree(p); } }
...
//-----------------------------------------------------------------------------
// ImGuiStyle
// You may modify the NuclearEngine::Graphics::ImGui::GetStyle() main instance during initialization and before NewFrame().
// During the frame, use NuclearEngine::Graphics::ImGui::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar() to alter the main style values,
// and NuclearEngine::Graphics::ImGui::PushStyleColor(ImGuiCol_XXX)/PopStyleColor() for colors.
//-----------------------------------------------------------------------------
...
// Access via NuclearEngine::Graphics::ImGui::GetIO(). Read 'Programmer guide' section in .cpp file for general usage.
//-----------------------------------------------------------------------------
...
struct ImGuiIO
{
    ...
    const char* LogFilename;                    // = "imgui_log.txt"// Path to .log file (default parameter to NuclearEngine::Graphics::ImGui::LogToFile when no file is specified).
    ...
    // You can obtain the ImDrawData* by calling NuclearEngine::Graphics::ImGui::GetDrawData() after Render(). See example applications if you are unsure of how to implement this.
...
namespace NuclearEngine 
{
	namespace Graphics 
	{
		namespace ImGui
		{
			...
			static inline bool  IsPosHoveringAnyWindow(const ImVec2&) { IM_ASSERT(0); return false; } // This was misleading and partly broken. You probably want to use the NuclearEngine::Graphics::ImGui::GetIO().WantCaptureMouse flag instead.
...
// Usage: static ImGuiOnceUponAFrame oaf; if (oaf) NuclearEngine::Graphics::ImGui::Text("This will be called only once per frame");
struct ImGuiOnceUponAFrame
{
    ImGuiOnceUponAFrame() { RefFrame = -1; }
    mutable int RefFrame;
    operator bool() const { int current_frame = NuclearEngine::Graphics::ImGui::GetFrameCount(); if (RefFrame == current_frame) return false; RefFrame = current_frame; return true; }
};
...
struct ImGuiStorage
{
    ...
    //      float* pvar = NuclearEngine::Graphics::ImGui::GetFloatRef(key); NuclearEngine::Graphics::ImGui::SliderFloat("var", pvar, 0, 100.0f); some_var += *pvar;
...
//             NuclearEngine::Graphics::ImGui::Text("line number %d", i);
...
struct ImColor
{
    ...
    inline operator ImU32() const                                   { return NuclearEngine::Graphics::ImGui::ColorConvertFloat4ToU32(Value); }
    inline operator ImVec4() const                                  { return Value; }

    // FIXME-OBSOLETE: May need to obsolete/cleanup those helpers.
    inline void    SetHSV(float h, float s, float v, float a = 1.0f){ NuclearEngine::Graphics::ImGui::ColorConvertHSVtoRGB(h, s, v, Value.x, Value.y, Value.z); Value.w = a; }
    static ImColor HSV(float h, float s, float v, float a = 1.0f)   { float r,g,b; NuclearEngine::Graphics::ImGui::ColorConvertHSVtoRGB(h, s, v, r, g, b); return ImColor(r,g,b,a); }
};
...
// Draw command list
// This is the low-level list of polygons that NuclearEngine::Graphics::ImGui:: functions are filling. At the end of the frame,
// all command lists are passed to your ImGuiIO::RenderDrawListFn function for rendering.
// Each dear imgui window contains its own ImDrawList. You can use NuclearEngine::Graphics::ImGui::GetWindowDrawList() to 
// access the current window draw list and draw custom primitives.
// You can interleave normal NuclearEngine::Graphics::ImGui:: calls and adding primitives to the current draw list.
// All positions are generally in pixel coordinates (top-left at (0,0), bottom-right at io.DisplaySize), but you are totally free to apply whatever transformation matrix to want to the data (if you apply such transformation you'll want to apply it to ClipRect as well)
// Important: Primitives are always added to the list and not culled (culling is done at higher-level by NuclearEngine::Graphics::ImGui:: functions), if you use this API a lot consider coarse culling your drawn objects.
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared draw data (you can use NuclearEngine::Graphics::ImGui::GetDrawListSharedData() to get the one from current ImGui context)
    ...
    // If you want to create ImDrawList instances, pass them NuclearEngine::Graphics::ImGui::GetDrawListSharedData() or create and use your own ImDrawListSharedData (so you can use ImDrawList without ImGui)
    ImDrawList(const ImDrawListSharedData* shared_data) { _Data = shared_data; _OwnerName = NULL; Clear(); }
    ~ImDrawList() { ClearFreeMemory(); }
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level NuclearEngine::Graphics::ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImFontAtlas
{
    ...
    bool                        Locked;             // Marked as Locked by NuclearEngine::Graphics::ImGui::NewFrame() so attempt to modify the atlas will assert.
    ...
    ImVector<ImFont*>           Fonts;              // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling NuclearEngine::Graphics::ImGui::NewFrame(), use NuclearEngine::Graphics::ImGui::PushFont()/PopFont() to change the current font.
