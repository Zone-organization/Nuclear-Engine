...
namespace NuclearEngine
{
	namespace Graphics
	{
		...
		bool D3DCompiler::Compile(Graphics::Shader *result, const std::string& SourceCode, LLGL::ShaderType type)
		{
		...
		void D3DCompiler::Reflect(Graphics::Shader *result, ID3D10Blob* blob)
		{
		...
		inline ShaderVariableType D3DCompiler::GetShaderVariableType(const D3D11_SHADER_TYPE_DESC & VarTypeDesc)
		{
			...
			case D3D_SVC_STRUCT:
				...
				/*for (UINT i = 0; i < VarTypeDesc.Members; i++)
				{
				auto Member = VarType->GetMemberTypeByIndex(i);
				ParseVariable(Member, true);
				}*/
				break;
			default:
		...
		void D3DCompiler::Check_D3DCompile_Errors(ID3D10Blob* Shader)
		{
			char* compileErrors;
		...
		void D3DCompiler::ParseStruct(const D3D11_SHADER_VARIABLE_DESC& sdesc, const D3D11_SHADER_TYPE_DESC& tdesc, Graphics::Shader *result)
		{
			//Check if we already parsed it.
			for (auto i : result->mReflection.mStructures)
			{
				if (i.mName == sdesc.Name)
					return;
			}

			ShaderStructure structure;
			structure.mName = sdesc.Name;
			//structure.
			result->mReflection.mStructures.push_back(structure);
		}
		void D3DCompiler::ParseConstantBuffers(const D3D11_SHADER_DESC& shaderDesc, ID3D11ShaderReflection* pReflector, Graphics::Shader *result)
		{
			for (Uint32 i = 0; i < shaderDesc.ConstantBuffers; i++)
			{
				...
				else
				{
					...
					for (Uint32 i = 0; i < CBDesc.Variables; i++)
					{
						...
						{
							Log.Error("[Reflect_DXBC] [ParseConstantBuffers] Cannot get constant buffer variable desc with index: " + std::to_string(i) + "\n");
						}
						else
						{
							//Parse Variables
							auto VarType = VariableReflection->GetType();
							D3D11_SHADER_TYPE_DESC VarTypeDesc;
							VarType->GetDesc(&VarTypeDesc);

							ShaderVariable reflected_var;
							reflected_var.mType = GetShaderVariableType(VarTypeDesc);
							reflected_var.mSize = VariableDesc.Size;
							reflected_var.mOffset = VarTypeDesc.Offset;

							if (reflected_var.mType == ShaderVariableType::Struct)
							{
								ParseStruct(VariableDesc, VarTypeDesc, result);
							}
		...
		void D3DCompiler::ParseResources(const D3D11_SHADER_DESC& shaderDesc, ID3D11ShaderReflection* pReflector, Graphics::Shader *result)
		{
