/*
Modified Work from
https://github.com/DiligentGraphics/DiligentFX/blob/master/Components/src/ShadowMapManager.cpp
...
namespace NuclearEngine
{
	namespace Systems
	{
			ShadowingSubSystem::ShadowingSubSystem()
			{

			}

			void ShadowingSubSystem::Initialize(IRenderDevice* pDevice, const InitInfo& initInfo)
			{
			...
			void ShadowingSubSystem::DistributeCascades(const DistributeCascadeInfo& Info,
				Shadowing::ShadowMapAttribs& ShadowAttribs)
			{
			...
			void ShadowingSubSystem::InitializeConversionTechniques(TEXTURE_FORMAT FilterableShadowMapFmt)
			{
			...
			void ShadowingSubSystem::InitializeResourceBindings()
			{
				for (int mode = SHADOW_MODE_VSM; mode <= SHADOW_MODE_EVSM4; ++mode)
				{
					auto& Tech = m_ConversionTech[mode];
			...
			void ShadowingSubSystem::ConvertToFilterable(IDeviceContext* pCtx, const Shadowing::ShadowMapAttribs& ShadowAttribs)
			{
				if (m_ShadowMode == SHADOW_MODE_VSM || m_ShadowMode == SHADOW_MODE_EVSM2 || m_ShadowMode == SHADOW_MODE_EVSM4)
				{
