		...
		bool Image::CreateTextureFromRawImage(const Assets::ImageData& src_data, const Importers::ImageLoadingDesc& Desc)
		{
			...
			else
			{
				...
				if (TexFmtDesc.NumComponents != NumComponents)
					LOG_ERROR_AND_THROW("Incorrect number of components ", src_data.mNumComponents, ") for texture format ", TexFmtDesc.Name);
			...
			if (src_data.mNumComponents == 3)
			{
				VERIFY_EXPR(NumComponents == 4);
				auto RGBAStride = src_data.mWidth * NumComponents * ChannelDepth / 8;
				RGBAStride = (RGBAStride + 3) & (-4);
				Mips[0].resize(size_t{ RGBAStride } *size_t{ src_data.mHeight });
				pSubResources[0].pData = Mips[0].data();
				pSubResources[0].Stride = RGBAStride;
				if (ChannelDepth == 8)
					RGBToRGBA<Uint8>(src_data.mData, src_data.mRowStride,
						Mips[0].data(), RGBAStride,
						src_data.mWidth, src_data.mHeight);
				else if (ChannelDepth == 16)
					RGBToRGBA<Uint16>(src_data.mData, src_data.mRowStride,
						Mips[0].data(), RGBAStride,
						src_data.mWidth, src_data.mHeight);
			}
			else
