    ...
    void ShadowMapManager::InitializeConversionTechniques(TEXTURE_FORMAT FilterableShadowMapFmt)
    {
        ...
        for (int mode = SHADOW_MODE_VSM; mode <= SHADOW_MODE_EVSM4; ++mode)
        {
            ...
            if (!pScreenSizeTriVS)
            {
                ...
                VertShaderCI.Source = VertShaderCI.Source = Nuclear::Core::FileSystem::LoadFileToString("Assets/NuclearEngine/Shaders/Shadowing/FullScreenTriangleVS.fx").c_str();
            ...
            ShaderCI.Source = Nuclear::Core::FileSystem::LoadFileToString("Assets/NuclearEngine/Shaders/Shadowing/ShadowConversions.fx").c_str();
