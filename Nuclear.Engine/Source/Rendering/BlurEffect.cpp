	...
	namespace Rendering
	{
		void BlurEffect::Initialize(Uint32 RTWidth, Uint32 RTHeight)
		{
			...
			LayoutElems.push_back(LayoutElement(1, 0, 2, VT_FLOAT32, false)); //UV
			{
				...
				auto source = Platform::FileSystem::GetInstance().LoadShader("@NuclearAssets@/Shaders/Blur.vs.hlsl", { "HORIZENTAL" }, std::set<std::string>(), true);
			...
			}
			{
				...
				auto source = Platform::FileSystem::GetInstance().LoadShader("@NuclearAssets@/Shaders/Blur.vs.hlsl", std::set<std::string>(), std::set<std::string>(), true);
				CreationAttribs.Source = source.c_str();
			...
			}
			{
				...
				auto source = Platform::FileSystem::GetInstance().LoadShader("@NuclearAssets@/Shaders/Blur.ps.hlsl", std::set<std::string>(), std::set<std::string>(), true);
				CreationAttribs.Source = source.c_str();
