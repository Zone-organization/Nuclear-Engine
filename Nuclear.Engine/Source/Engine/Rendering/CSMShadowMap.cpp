...
namespace Nuclear
{
	namespace Rendering
	{
            ...
            ShadowMapAttribs& ShadowAttribs)
        {
            ...
            Math::Vector2 f2ShadowMapSize = Math::Vector2(static_cast<float>(SMDesc.Width), static_cast<float>(SMDesc.Height));
            ...
            for (int iCascade = 0; iCascade < iNumCascades; ++iCascade)
            {
                ...
                Math::Vector2 f2FixedMargin = (Info.SnapCascades ? Math::Vector2(0.5f, 0.5f) : Math::Vector2(0, 0));
                if (m_ShadowMode == SHADOW_MODE_VSM || m_ShadowMode == SHADOW_MODE_EVSM2 || m_ShadowMode == SHADOW_MODE_EVSM4)
                {
                ...
                else
                {
                    ...
                    Math::Vector2          f2MaxExtension = 2.f * (Math::Vector2(MaxVaryingFilterRadius, MaxVaryingFilterRadius) + f2FixedMargin);
                    Math::Vector2          f2MinCascadeExtent = ShadowAttribs.fFilterWorldSize * (f2ShadowMapSize - f2MaxExtension) / MaxVaryingFilterRadius * NDCtoUVScale;
                    float3          f3NewExtent;
                    ...
                    Math::Vector2 K = Math::Vector2(ShadowAttribs.fFilterWorldSize, ShadowAttribs.fFilterWorldSize) / Math::Vector2(f3CascadeExtent.x, f3CascadeExtent.y) * NDCtoUVScale;
                    f2FilterMargin = K * (f2ShadowMapSize - 2.f * f2FixedMargin) / (Math::Vector2(1, 1) + 2.f * K);
                }

                Math::Vector2 f2Margin = f2FixedMargin + f2FilterMargin;
                Math::Vector2 f2Extension = f2Margin * 2.f;
            ...
            for (int mode = SHADOW_MODE_VSM; mode <= SHADOW_MODE_EVSM4; ++mode)
            {
                ...
                if (!pScreenSizeTriVS)
                {
                    ...
                    VertShaderCI.UseCombinedTextureSamplers = true;
                       auto trishadersource = Nuclear::Core::FileSystem::LoadFileToString("Assets/NuclearEngine/Shaders/CSM/FullScreenTriangleVS.fx");
                       VertShaderCI.Source = trishadersource.c_str();
                ...
                auto shadersource =  Nuclear::Core::FileSystem::LoadFileToString("Assets/NuclearEngine/Shaders/CSM/ShadowConversions.fx");
