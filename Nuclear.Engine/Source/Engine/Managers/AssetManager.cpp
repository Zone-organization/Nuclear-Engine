...
namespace Nuclear {
	namespace Managers {
		...
		void AssetManager::Initialize(AssetManagerDesc desc)
		{
			...
			{
				NUCLEAR_ERROR("[AssetManager : '{0}'] Failed To Initialize FreeType library");
			}
		}
		...
		Assets::Font* AssetManager::Import(const Core::Path& Path, const Importers::FontLoadingDesc& desc)
		{
			...
			if (msdfgen::FontHandle* font = msdfgen::loadFont(FT_Handle, Path.mRealPath.c_str())) {
				...
				NUCLEAR_ERROR("[AssetManager : '{0}'] Failed to import font : '{1}' : '{2}'", mDesc.mName, Path.mInputPath, Utilities::int_to_hex<Uint32>(hashedname));
			}
