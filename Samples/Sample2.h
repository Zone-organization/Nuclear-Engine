...
class Sample2 : public Client
{
	...
	//Rendering::DefferedRenderingPipeline DefferedRP;
...
public:
	...
	void LoadPBRMaterials()
	{
		...
		RustedIron.Create(&RustedIron_D, &PBR_IBL);
		Plastic.Create(&Plastic_D, &PBR_IBL);
		Grass.Create(&Grass_D, &PBR);
		Wall.Create(&Wall_D, &PBR);
		Gold.Create(&Gold_D, &PBR_IBL);
	}
	...
	void InitRenderer()
	{
		...
		Renderer->RegisterShadingModel(&PBR);
		Renderer->RegisterShadingModel(&PBR_IBL);
