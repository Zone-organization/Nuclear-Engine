...
PixelInputType main(VertexInputType input)
{
   ...
   // Some models have symetric UVs. Check and fix.
    ...
    for (int i0 = 0; i0 < DirSpotPointActiveCasters.x; i0++)
    {
	...
	for (int i1 = 0; i1 < DirSpotPointActiveCasters.y; i1++)
    {
        output.SpotLight_FragPos[i1] = mul(Spot_LightSpace[i1], float4(output.FragPos, 1.0f));
    }
#endif

    output.DirSpotPointActiveCasters = DirSpotPointActiveCasters;
#endif //NE_SHADOWS
