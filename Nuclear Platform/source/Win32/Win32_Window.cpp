...
namespace NuclearPlatform {
	...
	namespace Internals {
		...
		BOOL Win32_Window::InitWindow()
		{
			...
			wc.lpszClassName = L"NuclearPlatform";								// Set The Class Name
			...
			SetProp(m_Handle, L"NuclearPlatformProp", WindPropHandle);
			return TRUE;									// Success
		}
		...
		BOOL Win32_Window::CleanWindow()
		{
			...
			if (!UnregisterClass(L"NuclearPlatform", hInstance))			// Are We Able To Unregister Class
			{
		...
		{
			Win32_Window window = GetProp(hWnd, L"NuclearPlatformProp");
			PAINTSTRUCT ps;
