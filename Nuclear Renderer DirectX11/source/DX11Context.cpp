...
namespace NuclearRenderer {
	...
	bool DX11Context::Initialize()
	{
		...
		{
			if (displayModeList[i].Width == (unsigned int)Application::GetWidth())
			{
				if (displayModeList[i].Height == (unsigned int)Application::GetHeight())
				{
		...
		SwapChainDesc.OutputWindow = Application::GetInternalWindow()->GetHandle();
		...
		depthBufferDesc.Width = Application::GetInternalWindow()->GetWidth();
		depthBufferDesc.Height = Application::GetInternalWindow()->GetHeight();
		...
		viewPort.TopLeftX = 0;
		viewPort.TopLeftY = 0;
		viewPort.Width = Application::GetInternalWindow()->GetWidth();
		viewPort.Height = Application::GetInternalWindow()->GetHeight();
	...
	NRBVertexBuffer * DX11Context::ConstructVertexBuffer(NRBVertexBuffer * param)
	{
		param = new NuclearRenderer::DX11VertexBuffer();

		return param;
	}

	NRBIndexBuffer * DX11Context::ConstructIndexBuffer(NRBIndexBuffer * param)
	{
		param = new NuclearRenderer::DX11IndexBuffer();
	...
	NRBShader* DX11Context::ConstructShader(NRBShader * param)
	{
		param = new NuclearRenderer::DX11Shader();
